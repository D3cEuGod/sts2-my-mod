...
- For the current UI scope, size the main expanded panel from the rows actually rendered in the three primary sections, not from full live content bounds and not from a coarse fixed `maxRows` reservation. That keeps the HUD tighter during sparse states without letting fully dynamic content measurement overshoot.
- When making player rows easier to scan, prefer stable per-player visual identity (for example a deterministic tinted background by player id) over ad-hoc per-rank coloring that changes too much from fight to fight.
